...
*/
select customer.cus_name,customer.cus_gender from customer where customer.cus_name like'A%' or cus_name like '%A';
...
group by supplier_pricing.supp_id)as final
 ...
 use `order-directory`;
 call ratingprocedure();select count(t2.cus_gender) as NoOfCustomer, t2.cus_gender from (select t1.cus_id, t1.cus_gender, t1.ord_amount, t1.cus_name from (select `order` .*, customer.cus_gender, customer.cus_name from `order` inner join customer on `order`.cus_id=customer.cus_id having `order`.ord_amount>=3000) as t1 group by t1.cus_id) as t2 group by t2.cus_gender LIMIT 0, 1000
